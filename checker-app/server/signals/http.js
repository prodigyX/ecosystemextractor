...
export async function checkHttp(project) {
  ...
  try {
    ...
    evidence.push(ev('bad', 'Website unreachable', `${reason}`, W.unreachable))
    return { facts, evidence }
  }

  if (facts.status == null) {
    evidence.push(ev('bad', 'Redirect loop', `More than ${MAX_REDIRECTS} redirects`, W.redirectLoop))
    return { facts, evidence }
  }

  if (facts.status >= 200 && facts.status < 300) {
    evidence.push(ev('good', `Website up (HTTP ${facts.status})`, null, W.up))
  } else if (facts.status === 401 || facts.status === 403) {
    evidence.push(ev('warn', `Website responds but blocks bots (HTTP ${facts.status})`, null, W.botBlocked))
  } else if (facts.status >= 400 && facts.status < 500) {
    evidence.push(ev('bad', `Website client error (HTTP ${facts.status})`, null, W.clientError))
  } else if (facts.status >= 500) {
    evidence.push(ev('bad', `Website server error (HTTP ${facts.status})`, null, W.serverError))
  }
  ...
  if (facts.redirects.length > 0) {
    ...
    if (startDomain && endDomain && startDomain !== endDomain) {
      evidence.push(
        ev('warn', 'Redirects to different domain', `${startDomain} → ${endDomain}`, W.crossDomainRedirect)
      )
    } else {
      evidence.push(
